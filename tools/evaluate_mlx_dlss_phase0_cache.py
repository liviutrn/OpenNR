...
def main() -> None:
    ...
    with torch.no_grad():
        for row_index, row in selected:
            ...
            result = pipeline.enhance(source, frame_index=int(row["frame_id"]) - 1, **enhance_options)
            if torch.cuda.is_available():
                torch.cuda.synchronize()
            ...
            if determinism_max_abs is None:
                repeat = pipeline.enhance(source, frame_index=int(row["frame_id"]) - 1, **enhance_options)
