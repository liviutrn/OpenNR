"""Matched ordinary-1x native residual versus affine-only continuation."""
import argparse
import hashlib
import json
...
from native_residual_tone_head import load_warm_head
...
def main():
    ...
    p.add_argument('--native-residual', action='store_true')
    ...
    if a.native_residual != bool(a.control):
        raise ValueError('Native residual arm requires the completed matched control')
    ...
    head, warm = load_warm_head(a.warm_head, a.native_residual)
    ...
    run = dict(scientific, native_residual=a.native_residual,
               head_parameters=sum(p.numel() for p in head.parameters()),
               source_sha256={name: sha(Path(__file__).with_name(name)) for name in
                   ('train_native_residual_ablation.py', 'native_residual_tone_head.py',
    ...
    if a.control:
        ...
        if cr['native_residual']:
            raise ValueError('Control must use affine-only output')
