"""Train the local-window attention temporal ablation from v9."""
...
from attention_temporal_student import (
    AttentionConfig,
...
def _checkpoint(path, model, ema, optimizer, step, best_mae, best_feature, run, schedule):
    payload = {
        ...
        "attention_config": run["attention_config"],
...
def _load_parent(path: Path, device="cuda"):
    ...
    if saved.get("architecture") != "context_v5_temporal":
        raise ValueError("Attention phase expects a context_v5_temporal parent checkpoint")
    ...
    return model, saved, config, temporal, attention
...
def main() -> None:
    ...
    parser.add_argument("--history-warmup", action="store_true")
    parser.add_argument("--base-lr", type=float, default=1e-6)
    parser.add_argument("--temporal-lr", type=float, default=1e-5)
    ...
    parser.add_argument("--change-scale", type=float, default=3.0)
    parser.add_argument("--delta-weight", type=float, default=0.12)
    parser.add_argument("--spatial-probability", type=float, default=0.0)
    parser.add_argument("--attention-width", type=int, default=64)
    parser.add_argument("--attention-blocks", type=int, default=3)
    parser.add_argument("--attention-heads", type=int, default=4)
    parser.add_argument("--attention-window", type=int, default=8)
    parser.add_argument("--attention-scale", type=float, default=0.12)
    parser.add_argument("--shifted-windows", action="store_true")
    parser.add_argument(
        "--multi-scale-context",
        action="store_true",
        help="Add medium/coarse whole-eye token cross-attention to the refiner",
    )
    ...
    if args.history_warmup and args.batch != 1:
        raise ValueError("history warmup currently requires batch size one")
    ...
    if args.attention_lr < 0:
        raise ValueError("attention learning rate must not be negative")
    if args.change_scale < 0:
        raise ValueError("change scale must not be negative")
    ...
    model, parent, config, temporal, default_attention = _load_parent(parent_path)
    attention = AttentionConfig(
        width=args.attention_width,
        blocks=args.attention_blocks,
        heads=args.attention_heads,
        window=args.attention_window,
        delta_scale=args.attention_scale,
        shifted=args.shifted_windows,
        multi_scale=args.multi_scale_context,
    )
    if attention != default_attention:
        model = AttentionTemporalStyleContextStudent(config, temporal, attention).cuda()
        model.initialize_v5(parent["model"])
    ema = deepcopy(model)
    attention_params = [
    ...
    base_params = [
        parameter for name, parameter in model.named_parameters()
        if not name.startswith("attention_") and not name.startswith("temporal_")
    ]
    if not attention_params or not temporal_params or not base_params:
        raise ValueError("Attention-temporal parameter partition is empty")
    ...
    schedule = {
        ...
        "shifted_windows": args.shifted_windows,
        "multi_scale_context": args.multi_scale_context,
    ...
    run = {
        ...
        "attention_config": vars(attention),
        "parent_checkpoint": str(parent_path),
        "parent_sha256": parent_sha256,
        "parent_architecture": parent.get("architecture"),
        "parent_step": int(parent.get("step", -1)),
        "attention_contract": {
            "grid": "quarter-resolution 8x8 local windows",
            "conditioning": "RGB, current student prediction, exact guides, whole-eye context",
            "recovered_dlss_graph": False,
        },
        ...
        "parameter_partition": {
            ...
            "attention": sum(parameter.numel() for parameter in attention_params),
        },
        "schedule": schedule,
        "test_used": False,
    ...
    for step in range(1, args.steps + 1):
        ...
        if use_spatial:
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                ...
                loss = detail_loss(
                    prediction.float(), target, rgb, change_scale=args.change_scale
                )
            ...
            prefix = 0
            if args.history_warmup:
                prefix = int(starts[0])
                indices = cache.streams[selected[0]][: prefix + args.window][None, :]
            ...
            for frame in range(total_frames):
                local_frame = frame - prefix
                if frame < prefix or local_frame < args.burn_in:
                ...
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    ...
                    if appearance is not None and local_frame == args.window - 1:
    ...
    print("ATTENTION-TEMPORAL TRAINING COMPLETE", flush=True)
