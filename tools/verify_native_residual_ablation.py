...
import torch
from native_residual_tone_head import NativeResidualToneHead
...
from train_native_residual_ablation import cohort
...
def load_checkpoint(path):
    ...
    head = (NativeResidualToneHead() if run['native_residual'] else StableOversizedToneHead()).cuda()
...
def main():
    ...
    for label, root in zip(run['cohort_labels'], run['cohorts']):
        ...
        if a.render:
            ...
            folder.mkdir()
            entry['samples'] = render_samples(model, cache, 'Native residual' if run['native_residual'] else 'Affine control', folder)
