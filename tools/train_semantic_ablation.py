"""Matched frozen random versus pretrained DINOv2 feature conditioning on ordinary1x data."""
import argparse
import hashlib
import json
...
def main():
    ...
    p.add_argument('--pretrained-encoder', action='store_true')
    p.add_argument('--frozen-parent', action='store_true',
                   help='Train the pretrained semantic head while keeping the causal parent frozen.')
    ...
    if a.steps < 1 or a.eval_every < 1 or a.learning_rate <= 0:
    ...
    if a.frozen_parent and not a.pretrained_encoder:
        raise ValueError('--frozen-parent requires --pretrained-encoder')
    if not a.frozen_parent and a.pretrained_encoder != bool(a.control):
        raise ValueError('Pretrained encoder arm requires the completed matched control')
    if a.frozen_parent and a.control:
        raise ValueError('--frozen-parent is a standalone control and cannot take --control')
    ...
    head, warm = load_semantic_warm_head(a.warm_head, a.pretrained_encoder)
    ...
    run = dict(scientific, pretrained_encoder=a.pretrained_encoder,
               frozen_parent=bool(a.frozen_parent),
               head_parameters=sum(p.numel() for p in head.parameters()),
               source_sha256={name: sha(Path(__file__).with_name(name)) for name in
                   ('train_semantic_ablation.py', 'semantic_tone_head.py',
    ...
    if a.control:
        ...
        if cr['pretrained_encoder']:
            raise ValueError('Control must use the frozen random encoder')
    ...
    optimizer = torch.optim.AdamW(head.parameters(), lr=a.learning_rate, weight_decay=1e-4)
    history = []
    ...
    def checkpoint(name, step, metrics):
        ...
        torch.save({'head': head.state_dict(), 'run': run, 'step': step, 'validation': metrics}, tmp)
