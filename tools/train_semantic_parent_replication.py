"""Independent-seed paired frozen versus trainable causal-parent semantic comparison."""
import argparse
import hashlib
import json
...
def main():
    ...
    p.add_argument('--freeze-parent', action='store_true')
    p.add_argument('--steps', type=int, default=1200)
    p.add_argument('--seed', type=int, default=812)
    p.add_argument('--learning-rate', type=float, default=1e-4)
    p.add_argument('--eval-every', type=int, default=400)
    a = p.parse_args()
    if a.freeze_parent == bool(a.control):
        raise ValueError('Frozen control must omit --control; trainable arm requires it')
    if a.output.exists():
    ...
    model = FrozenParentToneModel(parent, head) if a.freeze_parent else JointParentToneModel(parent, head)
    ...
    run = dict(scientific, pretrained_encoder=True,
               ...
               source_sha256={name: sha(Path(__file__).with_name(name)) for name in
                   ('train_semantic_parent_replication.py', 'joint_parent_tone_model.py',
                    'train_spatial_tone.py', 'train_temporal_student.py', 'aligned_cohort.py', 'semantic_tone_head.py',
    ...
    run['joint_parent'] = not a.freeze_parent
    run['freeze_parent'] = a.freeze_parent
    run['parent_learning_rate'] = 0.0 if a.freeze_parent else a.parent_learning_rate
    ...
    optimizer = torch.optim.AdamW([{'params': [p for p in head.parameters() if p.requires_grad], 'lr': a.learning_rate},
                                  {'params': list(parent.parameters()), 'lr': run['parent_learning_rate']}], weight_decay=1e-4)
    history = []
