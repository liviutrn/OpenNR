"""Train a half-resolution detail refiner from the confirmed v9 student.

This phase is deliberately controlled: it starts from the best generalizing
v9 temporal checkpoint, keeps the strict reset-qualified cache and immutable
sequence splits, and gives the new detail branch its own learning-rate group.
Optional merged spatial examples are supported for a matched follow-up, but
the default probe uses strict clips only so the architecture effect is clear.
"""
...
from detail_temporal_student import DetailConfig, DetailTemporalStyleContextStudent
...
from train_capacity_temporal_student import SpatialCache
...
def _checkpoint(
    ...
    payload = {
        ...
        "detail_config": run["detail_config"],
...
def _load_parent(path: Path, device="cuda"):
    ...
    if saved.get("architecture") != "context_v5_temporal":
        raise ValueError("Detail phase expects a context_v5_temporal parent checkpoint")
    ...
    return model, saved, config, temporal, detail
...
def main() -> None:
    ...
    parser.add_argument("--detail-lr", type=float, default=2e-5)
    ...
    parser.add_argument("--detail-width", type=int, default=48)
    parser.add_argument("--detail-blocks", type=int, default=4)
    parser.add_argument("--detail-downsample", type=int, default=2)
    ...
    model, parent, config, temporal, default_detail = _load_parent(parent_path)
    detail = DetailConfig(
        width=args.detail_width,
        blocks=args.detail_blocks,
        downsample=args.detail_downsample,
        delta_scale=args.detail_scale,
    )
    if detail != default_detail:
        model = DetailTemporalStyleContextStudent(config, temporal, detail).cuda()
    ...
    base_params = [
        parameter for name, parameter in model.named_parameters()
        if not name.startswith("detail_") and not name.startswith("temporal_")
    ]
    if not detail_params or not temporal_params or not base_params:
        raise ValueError("Detail-temporal parameter partition is empty")
    optimizer = torch.optim.AdamW(
        [
            {"params": base_params, "lr": args.base_lr},
            {"params": temporal_params, "lr": args.temporal_lr},
            {"params": detail_params, "lr": args.detail_lr},
        ],
        weight_decay=1e-4,
    )
    ...
    schedule = {
        ...
        "detail_downsample": args.detail_downsample,
    ...
    run = {
        ...
        "parameter_partition": {
            ...
            "detail": sum(parameter.numel() for parameter in detail_params),
        },
        "schedule": schedule,
        "test_used": False,
    ...
    for step in range(1, args.steps + 1):
        ...
        if use_spatial:
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                prediction, _ = model.forward_temporal(
    ...
    print("DETAIL-TEMPORAL TRAINING COMPLETE", flush=True)
