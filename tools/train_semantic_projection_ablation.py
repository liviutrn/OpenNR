"""Projection-only frozen random versus pretrained DINOv2 conditioning on ordinary1x data."""
import argparse
import hashlib
import json
...
def main():
    ...
    if a.pretrained_encoder != bool(a.control):
    ...
    head.semantic_projection.requires_grad_(True)
    ...
    scientific = {'warm_head': str(a.warm_head.resolve()), 'warm_head_sha256': sha(a.warm_head),
                  ...
                  'projection_only': True, 'learning_rate': a.learning_rate, 'eval_every': a.eval_every, 'window': 8,
                  'burn_in': 2, 'batch': 1, 'loss': 'L1 + .5 pooled16 RGB L1 + .12 temporal error delta',
    ...
    run = dict(scientific, pretrained_encoder=a.pretrained_encoder,
               ...
               source_sha256={name: sha(Path(__file__).with_name(name)) for name in
                   ('train_semantic_projection_ablation.py', 'semantic_tone_head.py',
    ...
    optimizer = torch.optim.AdamW((p for p in head.parameters() if p.requires_grad), lr=a.learning_rate, weight_decay=1e-4)
    history = []
