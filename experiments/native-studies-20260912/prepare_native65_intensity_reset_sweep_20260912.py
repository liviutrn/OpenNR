"""Prepare one-reset native 65% outputs for an intensity-only visual sweep."""
...
ROOT = Path(r"C:\OpenNR\Native65IntensityResetSweep_20260912")
DLL = Path(
...
def main() -> None:
    ...
    for intensity in INTENSITIES:
        ...
        record = {
            ...
            "tuning": "intensity plus fixed local tone/structure 1.70",
            "scope": "one native call after process initialization; first-call reset state",
        }
        (output / "provenance.json").write_text(json.dumps(record, indent=2), encoding="utf-8")
        records.append(record)
    manifest = {
        "schema": "opennr-native65-intensity-reset-sweep-v1",
        "source_frame": str(CAPTURE),
        "work_dimensions": [WORK_WIDTH, WORK_HEIGHT],
        "guide_dimensions": [GUIDE_WIDTH, GUIDE_HEIGHT],
        "intensities": records,
        "purpose": "isolate model intensity from post-compositor residual strength",
    }
