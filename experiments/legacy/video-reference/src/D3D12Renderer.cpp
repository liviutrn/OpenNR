...
D3D12Renderer::~D3D12Renderer() {
    ...
    // DLSSNR shares the NGX core module with standard DLSS. Tear it down first
    // so its feature handles are released while the core is still alive.
    m_dlssNr.Shutdown();
    m_dlss.Shutdown();
    if (m_fenceEvent) CloseHandle(m_fenceEvent);
}

bool D3D12Renderer::Initialize(HWND hwnd,uint32_t sourceW,uint32_t sourceH,uint32_t outputW,uint32_t outputH,uint32_t gridW,uint32_t gridH,NVSDK_NGX_PerfQuality_Value quality,const std::filesystem::path& dlssNrRuntime) {
    ...
    m_dlssNrRuntimePath=dlssNrRuntime;
    ...
    InitializeDLSSNR(dlssNrRuntime);
    if(!CreateVideoResources()) return false;
    LOG("DLSS5 guide contract: compact CPU optical-flow grid expanded on GPU into full R16G16_FLOAT MVs + R8 bias; depth is written directly into the same R32_TYPELESS/D32_FLOAT resource passed to NGX; temporal reset on discontinuities and tuning changes.");
    return true;
}
...
bool D3D12Renderer::CreateDeviceAndSwapchain(HWND hwnd) {
    ...
    // A video player has no reason to present torn intermediate frames. Keep
    // desktop playback synchronized even when the display advertises VRR
    // tearing support; this also removes a common full-frame flicker source.
    m_allowTearing=false;
    LOG("Video present pacing: VSync (tearing disabled for stable playback)");
    ...
    sd.BufferCount=FrameCount;sd.SwapEffect=DXGI_SWAP_EFFECT_FLIP_DISCARD;sd.Scaling=DXGI_SCALING_STRETCH;sd.AlphaMode=DXGI_ALPHA_MODE_IGNORE;sd.Flags=0;
...
bool D3D12Renderer::CreateHeapsAndBackbuffers(){
    ...
    D3D12_DESCRIPTOR_HEAP_DESC dh{};dh.Type=D3D12_DESCRIPTOR_HEAP_TYPE_DSV;dh.NumDescriptors=2;
...
float4 PSDepth(V i):SV_Target{float d=saturate(T.SampleLevel(S,i.uv,0).r);d=pow(d,0.7);return float4(d,d,d,1);}
    ...
    float PSWriteDepth(V i):SV_Depth{float4 g=T.SampleLevel(S,i.uv+JitterUV,0);uint mode=(uint)round(clamp(Misc.x,0.0,3.0));return (mode==1||mode==2)?1.0:saturate(g.b);}
    struct GuideOut{float2 mv:SV_Target0;float bias:SV_Target1;};
    GuideOut PSExpandGuides(V i){float4 g=T.SampleLevel(S,i.uv+JitterUV,0);uint mode=(uint)round(clamp(Misc.x,0.0,3.0));GuideOut o;o.mv=(mode==1||mode==3)?float2(0.0,0.0):g.xy;o.bias=mode==1?1.0:(g.w>=0.5?1.0:0.0);return o;}
)";
...
bool D3D12Renderer::InitializeDLSSNR(const std::filesystem::path& runtimePath) {
    const bool probed = runtimePath.empty() ? m_dlssNr.Probe() : m_dlssNr.Probe(runtimePath);
    if (!probed) {
        LOG("DLSSNR runtime unavailable: " << m_dlssNr.Detail());
        return false;
    }
    wchar_t exePath[MAX_PATH]{};
    GetModuleFileNameW(nullptr, exePath, static_cast<DWORD>(std::size(exePath)));
    const auto cachePath = std::filesystem::path(exePath).parent_path() / L"ngx_nr_cache";
    if (!m_dlssNr.Initialize(m_device.Get(), cachePath)) {
        LOG("DLSSNR initialization failed: " << m_dlssNr.Detail()
            << " result=0x" << std::hex << m_dlssNr.NgxResult() << std::dec);
        return false;
    }
    LOG("DLSSNR ready: runtime=" << m_dlssNr.Path().string()
        << " version=" << m_dlssNr.Version()
        << " appId=0x" << std::hex << m_dlssNr.ApplicationId()
        << " api=0x" << m_dlssNr.ApiVersion() << std::dec);
    return true;
}
...
bool D3D12Renderer::CreateVideoResources(){
    ...
    D3D12_DEPTH_STENCIL_VIEW_DESC dsv{};dsv.Format=DXGI_FORMAT_D32_FLOAT;dsv.ViewDimension=D3D12_DSV_DIMENSION_TEXTURE2D;m_device->CreateDepthStencilView(m_depth.Get(),&dsv,DSV(0));
    ...
    if(!HR(m_device->CreateCommittedResource(&hp,D3D12_HEAP_FLAG_NONE,&out,D3D12_RESOURCE_STATE_UNORDERED_ACCESS,nullptr,IID_PPV_ARGS(&m_neuralOutput)),"Create DLSSNR output"))return false;
    m_neuralOutput->SetName(L"DLSSNR_Output_Linear_FP16_UAV");
    m_device->CreateShaderResourceView(m_neuralOutput.Get(),&srv,SRVCPU(7));

    // Feature 18 accepts a same-size contract on the supplied runtime. During
    // a normal upscale, standard DLSS first produces m_dlssOutput and these
    // guides are regenerated at that final size before NR evaluates.
    auto nrMotion=Tex2D(DXGI_FORMAT_R16G16_FLOAT,m_outputW,m_outputH,D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET);
    if(!HR(m_device->CreateCommittedResource(&hp,D3D12_HEAP_FLAG_NONE,&nrMotion,D3D12_RESOURCE_STATE_RENDER_TARGET,nullptr,IID_PPV_ARGS(&m_neuralMotion)),"Create final-size NR motion guide"))return false;
    m_neuralMotion->SetName(L"DLSSNR_MotionVectors_FinalSize_RG16F");
    srv.Format=DXGI_FORMAT_R16G16_FLOAT;m_device->CreateRenderTargetView(m_neuralMotion.Get(),nullptr,RTV(6));

    auto nrBias=Tex2D(DXGI_FORMAT_R8_UNORM,m_outputW,m_outputH,D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET);
    if(!HR(m_device->CreateCommittedResource(&hp,D3D12_HEAP_FLAG_NONE,&nrBias,D3D12_RESOURCE_STATE_RENDER_TARGET,nullptr,IID_PPV_ARGS(&m_neuralBias)),"Create final-size NR bias guide"))return false;
    m_neuralBias->SetName(L"DLSSNR_Bias_FinalSize_R8");
    srv.Format=DXGI_FORMAT_R8_UNORM;m_device->CreateRenderTargetView(m_neuralBias.Get(),nullptr,RTV(7));

    D3D12_CLEAR_VALUE nrDcv{};nrDcv.Format=DXGI_FORMAT_D32_FLOAT;nrDcv.DepthStencil.Depth=1.0f;
    auto nrDepth=Tex2D(DXGI_FORMAT_R32_TYPELESS,m_outputW,m_outputH,D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL);
    if(!HR(m_device->CreateCommittedResource(&hp,D3D12_HEAP_FLAG_NONE,&nrDepth,D3D12_RESOURCE_STATE_DEPTH_WRITE,&nrDcv,IID_PPV_ARGS(&m_neuralDepth)),"Create final-size NR depth guide"))return false;
    m_neuralDepth->SetName(L"DLSSNR_Depth_FinalSize_R32_TYPELESS");
    D3D12_DEPTH_STENCIL_VIEW_DESC nrDsv{};nrDsv.Format=DXGI_FORMAT_D32_FLOAT;nrDsv.ViewDimension=D3D12_DSV_DIMENSION_TEXTURE2D;
    m_device->CreateDepthStencilView(m_neuralDepth.Get(),&nrDsv,DSV(1));
    LOG("DLSS resource contract ready: Color=R16G16B16A16_FLOAT " << m_renderW << "x" << m_renderH
        ...
        << ", DLSSNR.Output=R16G16B16A16_FLOAT UAV " << m_outputW << "x" << m_outputH
        << ", DLSSNR final guides=RG16F/R32_TYPELESS " << m_outputW << "x" << m_outputH
...
bool D3D12Renderer::RenderFrame(const uint8_t*bgra,size_t bytes,const float*guideGridRGBA32F,size_t guideBytes,uint32_t gridW,uint32_t gridH,bool temporalReset,float frameTimeMs){
    ...
    // Decoded video is already a sampled image; adding a synthetic camera-jitter
    // sequence makes static details shimmer and can look like frame flicker. Keep
    // the video path temporally stable and pass zero jitter to both DLSS stages.
    ...
    const float guidanceMode=static_cast<float>(std::min<std::uint32_t>(m_dlssNrTuning.frameGuidance,3u));
    ...
    cmd->SetGraphicsRootSignature(m_rootSig.Get());cmd->SetPipelineState(m_psoExpandGuides.Get());cmd->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);cmd->SetGraphicsRootDescriptorTable(0,SRVGPU(6));float guideParams[4]={jitterUVX,jitterUVY,guidanceMode,0};cmd->SetGraphicsRoot32BitConstants(1,4,guideParams,0);cmd->DrawInstanced(3,1,0,0);
    ...
    const bool nrRequested = NeuralRenderingEnabled() && m_dlssNr.IsInitialized();
    const bool nrNeedsUpscaleStage = nrRequested &&
        (m_renderW != m_outputW || m_renderH != m_outputH);
    const bool standardNeeded = DLSSEnabled() ||
        (nrNeedsUpscaleStage && m_dlss.Available());
    bool needFeatureFlush = false;
    if (standardNeeded && !m_dlss.FeatureCreated() && m_framesPresented >= 2) {
    ...
    // The supplied DLSSNR runtime is a separate NGX feature (feature 18). The
    // runtime accepts a reliable same-size contract, while the normal DLSS SR
    // path owns the input->output resolution change. In NR mode, run SR first
    // when needed, regenerate final-size guides, then evaluate Feature 18 on
    // that final-size color/depth/MV set.
    const bool preferNr = nrRequested;
    const bool evaluationReset = temporalReset || m_neuralTuningDirty;
    bool used=false;
    bool usedNr=false;
    // If NR needs a resolution change, standard DLSS is the first stage. Keep
    // its output in NON_PIXEL_SHADER_RESOURCE so it can immediately become the
    // same-size color input to Feature 18.
    const bool runStandardForNr = preferNr && nrNeedsUpscaleStage &&
        m_dlss.Available() && m_dlss.FeatureCreated();
    if (runStandardForNr) {
        if (m_dlssOutputInGuideRead) {
            Barrier(cmd,m_dlssOutput.Get(),GuideReadState,D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
            m_dlssOutputInGuideRead=false;
            m_outputInUAV=true;
        } else if (!m_outputInUAV) {
            Barrier(cmd,m_dlssOutput.Get(),D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE,D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
            m_outputInUAV=true;
        }
        used=m_dlss.Evaluate(cmd,m_dlssColor.Get(),m_dlssOutput.Get(),m_depth.Get(),m_motion.Get(),m_biasCurrent.Get(),evaluationReset,frameTimeMs,jitterX,jitterY);
        if(used){
            Barrier(cmd,m_dlssOutput.Get(),D3D12_RESOURCE_STATE_UNORDERED_ACCESS,GuideReadState);
            m_outputInUAV=false;
            m_dlssOutputInGuideRead=true;
        }
    }

    const bool nrCanEvaluate = preferNr && (!nrNeedsUpscaleStage || used);
    if (nrCanEvaluate) {
        ID3D12Resource* nrColor=m_dlssColor.Get();
        ID3D12Resource* nrDepth=m_depth.Get();
        ID3D12Resource* nrMotion=m_motion.Get();
        uint32_t nrWidth=m_renderW;
        uint32_t nrHeight=m_renderH;

        if (nrNeedsUpscaleStage) {
            // Expand the compact guide grid once more at output resolution. The
            // final-size resources are intentionally separate: NGX validates
            // each resource's dimensions against the Feature 18 contract.
            if(!m_neuralGuidesInRT){
                Barrier(cmd,m_neuralMotion.Get(),GuideReadState,D3D12_RESOURCE_STATE_RENDER_TARGET);
                Barrier(cmd,m_neuralBias.Get(),GuideReadState,D3D12_RESOURCE_STATE_RENDER_TARGET);
            }
            m_neuralGuidesInRT=true;
            D3D12_VIEWPORT fvp{0,0,float(m_outputW),float(m_outputH),0,1};
            D3D12_RECT fsc{0,0,LONG(m_outputW),LONG(m_outputH)};
            cmd->RSSetViewports(1,&fvp);cmd->RSSetScissorRects(1,&fsc);
            D3D12_CPU_DESCRIPTOR_HANDLE frt[2]={RTV(6),RTV(7)};
            cmd->OMSetRenderTargets(2,frt,FALSE,nullptr);
            cmd->SetGraphicsRootSignature(m_rootSig.Get());
            cmd->SetPipelineState(m_psoExpandGuides.Get());
            cmd->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
            cmd->SetGraphicsRootDescriptorTable(0,SRVGPU(6));
            float finalGuideParams[4]={jitterX/float(m_outputW),jitterY/float(m_outputH),guidanceMode,0};
            cmd->SetGraphicsRoot32BitConstants(1,4,finalGuideParams,0);
            cmd->DrawInstanced(3,1,0,0);
            Barrier(cmd,m_neuralMotion.Get(),D3D12_RESOURCE_STATE_RENDER_TARGET,GuideReadState);
            Barrier(cmd,m_neuralBias.Get(),D3D12_RESOURCE_STATE_RENDER_TARGET,GuideReadState);
            m_neuralGuidesInRT=false;

            if(!m_neuralDepthInWrite)
                Barrier(cmd,m_neuralDepth.Get(),DepthGuideReadState,D3D12_RESOURCE_STATE_DEPTH_WRITE);
            m_neuralDepthInWrite=true;
            auto finalDsv=DSV(1);
            cmd->RSSetViewports(1,&fvp);cmd->RSSetScissorRects(1,&fsc);
            cmd->OMSetRenderTargets(0,nullptr,FALSE,&finalDsv);
            cmd->ClearDepthStencilView(finalDsv,D3D12_CLEAR_FLAG_DEPTH,1.0f,0,0,nullptr);
            cmd->SetGraphicsRootSignature(m_rootSig.Get());
            cmd->SetPipelineState(m_psoDepthWrite.Get());
            cmd->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
            cmd->SetGraphicsRootDescriptorTable(0,SRVGPU(6));
            cmd->SetGraphicsRoot32BitConstants(1,4,finalGuideParams,0);
            cmd->DrawInstanced(3,1,0,0);
            Barrier(cmd,m_neuralDepth.Get(),D3D12_RESOURCE_STATE_DEPTH_WRITE,DepthGuideReadState);
            m_neuralDepthInWrite=false;

            nrColor=m_dlssOutput.Get();
            nrDepth=m_neuralDepth.Get();
            nrMotion=m_neuralMotion.Get();
            nrWidth=m_outputW;
            nrHeight=m_outputH;
        }

        if(!m_neuralOutputInUAV)
            Barrier(cmd,m_neuralOutput.Get(),D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE,D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
        m_neuralOutputInUAV=true;
        usedNr=m_dlssNr.Execute(cmd,0,nrColor,nrDepth,nrMotion,m_neuralOutput.Get(),
            nrWidth,nrHeight,m_outputW,m_outputH,m_dlssNrTuning.motionScaleX,m_dlssNrTuning.motionScaleY,
            m_dlssNrTuning,evaluationReset);
        if(usedNr){
            Barrier(cmd,m_neuralOutput.Get(),D3D12_RESOURCE_STATE_UNORDERED_ACCESS,D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
            m_neuralOutputInUAV=false;
        }
        if (usedNr && m_dlssNr.SuccessfulFrames() == 1)
            LOG("[DLSSNR] renderer dispatch completed on first frame");
        if(!usedNr && m_framesPresented <= 3)
            LOG("DLSSNR frame failed: " << m_dlssNr.Detail() << "; keeping the presentation path alive with the available DLSS/fallback output.");
    } else if (preferNr && nrNeedsUpscaleStage && m_framesPresented <= 2) {
        LOG("[DLSSNR] waiting for standard DLSS feature before the final-size NR stage");
    }

    // In Auto mode a failed same-size NR evaluation may still fall back to
    // conventional DLSS. The upscaled NR path already evaluated standard DLSS
    // above, so it must not be evaluated twice in the same command list.
    const bool useStandardFallback = !usedNr && !runStandardForNr &&
        DLSSEnabled() && m_dlss.FeatureCreated();
    ...
    // A failed NR pass after a successful SR first stage leaves the SR output in
    // guide-read state. It still needs a pixel-readable transition for the
    // Desktop presentation fallback.
    if (!usedNr && used && m_dlssOutputInGuideRead) {
        Barrier(cmd,m_dlssOutput.Get(),GuideReadState,D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
        m_dlssOutputInGuideRead=false;
    }

    m_lastDLSSUsed=used;
    m_lastDLSSNRUsed=usedNr;
    if (used || usedNr)
        m_neuralTuningDirty=false;
    ...
    switch(m_debugView){
        ...
        default:cmd->SetPipelineState(m_psoPresent.Get());if(usedNr)cmd->SetGraphicsRootDescriptorTable(0,SRVGPU(7));else if(used)cmd->SetGraphicsRootDescriptorTable(0,SRVGPU(1));else{debugPixelResource=m_dlssColor.Get();cmd->SetGraphicsRootDescriptorTable(0,SRVGPU(4));}break;
    }
...
bool D3D12Renderer::PresentCurrent(){
    ...
    switch(m_debugView){
        ...
        default:
            ...
            if(m_lastDLSSNRUsed&&DLSSNRAvailable())cmd->SetGraphicsRootDescriptorTable(0,SRVGPU(7));
            else if(m_lastDLSSUsed&&DLSSEnabled())cmd->SetGraphicsRootDescriptorTable(0,SRVGPU(1));
...
D3D12_CPU_DESCRIPTOR_HANDLE D3D12Renderer::DSV(uint32_t i)const{auto h=m_dsvHeap->GetCPUDescriptorHandleForHeapStart();h.ptr+=SIZE_T(i)*m_dsvInc;return h;}
