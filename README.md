...
OpenNR is a VR neural-rendering development project combining the native Open Shaders integration, synchronized capture, and separate model research. **Adaptive NR and adaptive Crop are part of the main development runtime in 2.15.1.** Both retain their existing opt-in defaults; saved user settings are not rewritten.
...
- [2.15.1 changes](runtime/open-shaders/package/OPENNR-2.15.1-CHANGELOG.md).
- [2.15.1 validation and local update](docs/OPENNR_2.15.1_RELEASE_20260917.md).
...
& 'E:\OpenNR_Builds\2.15.1\tests\cpp\Release\cpp_tests.exe'
```

The standard local archive is `E:\OpenNR_Builds\2.15.1\dist\OpenNR 2.15.1.7z`.
...
2.15.1 has source/build/package validation. The adaptive resource-envelope fallback is retained: rejection holds further crop tier changes until restart; NR can continue adapting. Live stereo, transitions, temporal appearance, frame pacing and sustained headset performance remain separate acceptance checks.

No learned model, recovered teacher, or generated target set is promoted. Native Feature 18 resources and capture provenance remain authoritative. The 2.15.1 AIO was installed in the local MGO profile with a separate rollback copy; no remote release was published.
