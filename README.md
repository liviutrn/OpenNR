...
OpenNR does not ship the upstream Open Shaders or Community Shaders logos. Those marks are non-GPL and are not redistributed by this fork. Action icons and category icons are bundled as before; the upstream Discord banner has been removed since the fork has no affiliated Discord channel. The menu uses the original OpenNR logo assets under the separate policy in `.github/assets/logo/LICENSE`.

OpenNR's own logo ([.github/assets/logo](.github/assets/logo)), used in the FOMOD installer header, is project-specific artwork and is not part of the GPL source license. See the LICENSE file in that directory.
